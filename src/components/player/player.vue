<template>
  <div class="player" v-show="playlist.length>0">
    ...
    <transition name="mini">
      <div class="mini-player" @click="open" ref="mini-player" v-show="!fullScreen">
        <div class="icon">
  ...
  export default {
    ...
    methods: {
      ...
      leave(el, done) {
        ...
        setTimeout(done, 400)  //需要添加延时0.4s在执行谈出动画
        animations.runAnimation(this.$refs.cdWrapper, 'move1')
      },
      afterleave() {
        // console.log('afterleave')
        animations.unregisterAnimation('move1')
      },
      togglePlaying() {
        if (!this.songReady) {
          return
      ...
      _getPosAndScale() {
        const cdInfo = this.$refs.cdWrapper.getBoundingClientRect()
        const miniInfo = this.$refs.imgWrapper.getBoundingClientRect()
        // console.log(cdInfo)
